...
public:
    ...
    {
        // do not re-allocate if equal number of voxels
        if ( mNumElem == w * h * d )
        {
            mWidth = w;
            mHeight = h;
    ...
    // won't try to free data
    void fromSharedData( T *data, unsigned width, unsigned height, unsigned depth )
    {
        freeData();
    ...
    void loadItkImage( ItkImageType* img, bool keepOnDestruction = false )
	{
		freeData();
		
		typename ItkImageType::IndexType index = img->GetLargestPossibleRegion().GetIndex();
    ...
    {
        typename ItkImageType::IndexType index =  ptr->GetLargestPossibleRegion().GetIndex();
